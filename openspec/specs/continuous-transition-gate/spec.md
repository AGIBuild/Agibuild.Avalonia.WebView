...
Closeout transition governance MUST assert that `completedPhase` and `activePhase` metadata remain semantically consistent with machine-checkable roadmap transition state, and MUST remain consistent with lane provenance context.
