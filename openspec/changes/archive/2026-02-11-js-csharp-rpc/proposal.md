...
## Problem
The current WebMessage bridge only supports raw string message passing. Consumers must manually serialize/deserialize, match request/response, and handle errors. This is the key gap vs Electron's `ipcMain`/`ipcRenderer`.
