...
项目的主目标不是“支持更多 UI 框架”，而是成为 **web-first 替代方案**：  
用 AI Agent 更擅长的 Web 技术栈（TypeScript/React/Vue + typed contracts）构建跨平台桌面应用，并系统性解决打包式桌面栈在工程实践中的高频痛点。
...
- 将本变更定位为 **web-first 替代架构基线**，以“Web-first + typed capability + policy-first”为主线。
...
- 本变更不一次性实现打包式桌面栈全生态能力（installer、auto-update、插件市场等）。
- 不改变现有平台 adapter 的基础行为语义（导航、桥接、权限、下载等）。

## Capabilities

### New Capabilities
- `phase5-foundation`: 面向 web-first 痛点的架构基线能力（typed bridge/capability/policy/automation）。
