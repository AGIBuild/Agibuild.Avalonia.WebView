...
Electron 迁移场景里，菜单/托盘/系统集成是“能不能上线真实桌面产品”的分水岭；当前基线已完成 typed bridge + capability + policy，但这三类壳能力尚未形成统一、可治理、可自动化验证的主干路径。  
现在补齐该路径，可直接承接 `ROADMAP` 中“应用壳能力扩展（菜单/托盘/系统集成）”并延续 Phase 5 的 Electron 痛点闭环目标。
...
- 不追求 Electron 全 API 对齐（如 auto-update、插件系统、安装器生态）。
...
- **Roadmap alignment**: 承接 `ROADMAP` Phase 5（Electron replacement foundation）成果并进入其 Next-phase Outlook 的“应用壳能力扩展（菜单/托盘/系统集成）”。
- **Affected areas**:
