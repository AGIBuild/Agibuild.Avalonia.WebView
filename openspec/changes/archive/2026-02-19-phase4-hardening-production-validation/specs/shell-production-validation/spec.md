...
The repository SHALL include a machine-readable shell production matrix artifact that declares shell capabilities, platform coverage, and executable evidence mappings.
...
Shell production soak scenarios SHALL be listed in runtime critical-path governance so regressions fail release-readiness checks early.

#### Scenario: Missing shell soak critical-path scenario fails governance
- **WHEN** required shell soak scenario IDs are missing from the runtime critical-path manifest
