...
The Core assembly SHALL define `IWindowsWebView2PlatformHandle` extending `IPlatformHandle` with:
...
- **WHEN** a consumer casts the `IPlatformHandle` to `IWindowsWebView2PlatformHandle`
...
The Core assembly SHALL define `IAppleWKWebViewPlatformHandle` extending `IPlatformHandle` with:
...
The Core assembly SHALL define `IGtkWebViewPlatformHandle` extending `IPlatformHandle` with:
...
The Core assembly SHALL define `IAndroidWebViewPlatformHandle` extending `IPlatformHandle` with:
