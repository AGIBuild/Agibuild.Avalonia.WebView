...
{
    private const int HostCapabilityDiagnosticSchemaVersion = 2;
    private const int SessionProfileDiagnosticSchemaVersion = 1;
    ...
    [Fact]
    public void Nuget_package_smoke_project_uses_local_feed_contract()
    {
        var repoRoot = FindRepoRoot();
        var projectPath = Path.Combine(repoRoot, "tests", "Agibuild.Fulora.Integration.NugetPackageTests", "Agibuild.Fulora.Integration.NugetPackageTests.csproj");
        var nugetConfigPath = Path.Combine(repoRoot, "tests", "Agibuild.Fulora.Integration.NugetPackageTests", "nuget.config");

        AssertFileExists(projectPath, RuntimeCriticalPathScenarioPresence);
        AssertFileExists(nugetConfigPath, RuntimeCriticalPathScenarioPresence);

        var projectSource = File.ReadAllText(projectPath);
        var nugetConfigSource = File.ReadAllText(nugetConfigPath);

        AssertSourceContains(projectSource, "<AgibuildPackageVersion Condition=\"'$(AgibuildPackageVersion)' == ''\">*-*</AgibuildPackageVersion>", RuntimeCriticalPathScenarioPresence, projectPath);
        AssertSourceContains(projectSource, "<NuGetAudit>false</NuGetAudit>", RuntimeCriticalPathScenarioPresence, projectPath);
        AssertSourceContains(projectSource, "EnsureLocalAgibuildPackageSourceExists", RuntimeCriticalPathScenarioPresence, projectPath);
        AssertSourceContains(projectSource, "<MakeDir Directories=\"$(LocalAgibuildPackageSource)\" />", RuntimeCriticalPathScenarioPresence, projectPath);
        AssertSourceContains(nugetConfigSource, "<add key=\"local-packages\" value=\"../../artifacts/packages\" />", RuntimeCriticalPathScenarioPresence, nugetConfigPath);
        AssertSourceContains(nugetConfigSource, "<add key=\"nuget.org\" value=\"https://api.nuget.org/v3/index.json\" />", RuntimeCriticalPathScenarioPresence, nugetConfigPath);
        Assert.DoesNotContain("<packageSourceMapping>", nugetConfigSource, StringComparison.Ordinal);
    }

    [Fact]
    public void Central_package_management_pins_safe_Tmds_DBus_Protocol_version_for_desktop_restore()
    {
        var version = ExtractCpmVersion("Tmds.DBus.Protocol");
        Assert.Equal("0.92.0", version);
    }

    [Fact]
    public void Package_build_transitive_targets_select_trim_roots_by_target_rid_not_host_os()
    {
        var repoRoot = FindRepoRoot();
        var targetsPath = Path.Combine(repoRoot, "src", "Agibuild.Fulora.Avalonia", "buildTransitive", "Agibuild.Fulora.Avalonia.targets");

        AssertFileExists(targetsPath, RuntimeCriticalPathScenarioPresence);

        var targetsSource = File.ReadAllText(targetsPath);

        AssertSourceContains(targetsSource, "<AgibuildFuloraTargetRuntimeIdentifier", RuntimeCriticalPathScenarioPresence, targetsPath);
        AssertSourceContains(targetsSource, "$(RuntimeIdentifier)", RuntimeCriticalPathScenarioPresence, targetsPath);
        AssertSourceContains(targetsSource, "StartsWith('osx')", RuntimeCriticalPathScenarioPresence, targetsPath);
        AssertSourceContains(targetsSource, "StartsWith('win')", RuntimeCriticalPathScenarioPresence, targetsPath);
        AssertSourceContains(targetsSource, "StartsWith('linux')", RuntimeCriticalPathScenarioPresence, targetsPath);
        AssertSourceContains(targetsSource, "StartsWith('android')", RuntimeCriticalPathScenarioPresence, targetsPath);
        Assert.DoesNotContain("<TrimmerRootAssembly Include=\"Agibuild.Fulora.Adapters.MacOS\"\n                         Condition=\"$([MSBuild]::IsOSPlatform('osx'))\"", targetsSource, StringComparison.Ordinal);
        Assert.DoesNotContain("<TrimmerRootAssembly Include=\"Agibuild.Fulora.Adapters.Windows\"\n                         Condition=\"$([MSBuild]::IsOSPlatform('windows'))\"", targetsSource, StringComparison.Ordinal);
        Assert.DoesNotContain("<TrimmerRootAssembly Include=\"Agibuild.Fulora.Adapters.Gtk\"\n                         Condition=\"$([MSBuild]::IsOSPlatform('linux'))\"", targetsSource, StringComparison.Ordinal);
    }
    ...
    public void Host_capability_diagnostic_contract_and_external_open_path_remain_schema_stable()
    {
        ...
        var bridgeSchemaMarkers = new[]
        {
            ...
            $"CurrentDiagnosticSchemaVersion = {HostCapabilityDiagnosticSchemaVersion}",
        ...
        var profileMarkers = new[]
        {
            ...
            $"CurrentDiagnosticSchemaVersion = {SessionProfileDiagnosticSchemaVersion}",
            "public int DiagnosticSchemaVersion { get; }",
    ...
    public void Ci_targets_remove_legacy_spec_strict_governance_gate_but_keep_release_governance_closure()
    {
        ...
        foreach (var target in requiredTargets)
            AssertTargetDeclarationExists(combinedSource, target, ReleaseOrchestrationDecisionGate, "build/Build*.cs");

        AssertStringLiteralExists(combinedSource, "dependency-governance-report.json", ReleaseOrchestrationDecisionGate, "build/Build*.cs");
        AssertStringLiteralExists(combinedSource, "typescript-governance-report.json", ReleaseOrchestrationDecisionGate, "build/Build*.cs");
        AssertStringLiteralExists(combinedSource, "sample-template-package-reference-governance-report.json", ReleaseOrchestrationDecisionGate, "build/Build*.cs");
        AssertStringLiteralExists(combinedSource, "closeout-snapshot.json", ReleaseOrchestrationDecisionGate, "build/Build*.cs");
        AssertStringLiteralExists(combinedSource, "transition-gate-governance-report.json", ReleaseOrchestrationDecisionGate, "build/Build*.cs");
        ...
        AssertTargetDependsOnContainsAll(mainSource, "Ci", ciDirectDependencies, ReleaseOrchestrationDecisionGate, "build/Build.cs");
        ...
        foreach (var dependency in ciRequiredClosureDependencies)
        {
            Assert.True(
                ...
                $"[{ReleaseOrchestrationDecisionGate}] Missing Ci transitive dependency '{dependency}'.");
        }

        Assert.False(
            ciClosure.Contains(legacyGateName),
            $"[{ReleaseOrchestrationDecisionGate}] Ci transitive dependency closure should not contain '{legacyGateName}'.");

        var ciPublishDependencies = new[] { "Ci", "Publish" };
        AssertTargetDependsOnContainsAll(mainSource, "CiPublish", ciPublishDependencies, ReleaseOrchestrationDecisionGate, "build/Build.cs");
    }
    ...
    [Fact]
    public void Linux_nativeaot_publish_uses_prepared_local_package_smoke_feed()
    {
        var repoRoot = FindRepoRoot();
        var ciWorkflowPath = Path.Combine(repoRoot, ".github", "workflows", "ci.yml");
        var buildCsPath = Path.Combine(repoRoot, "build", "Build.cs");
        var packagingPath = Path.Combine(repoRoot, "build", "Build.Packaging.cs");

        AssertFileExists(ciWorkflowPath, RuntimeCriticalPathScenarioPresence);
        AssertFileExists(buildCsPath, RuntimeCriticalPathScenarioPresence);
        AssertFileExists(packagingPath, RuntimeCriticalPathScenarioPresence);

        var ciWorkflow = File.ReadAllText(ciWorkflowPath);
        var buildCs = File.ReadAllText(buildCsPath);
        var packagingSource = File.ReadAllText(packagingPath);

        AssertSourceContains(ciWorkflow, "Run Linux NativeAOT NuGet smoke publish", RuntimeCriticalPathScenarioPresence, ciWorkflowPath);
        AssertSourceContains(ciWorkflow, "./build.sh --target ValidateLinuxNativeAotNugetPackagePublish --configuration Release --version-suffix", RuntimeCriticalPathScenarioPresence, ciWorkflowPath);
        AssertSourceContains(ciWorkflow, "sudo apt-get install -y libwebkit2gtk-4.1-dev libgtk-3-dev clang zlib1g-dev", RuntimeCriticalPathScenarioPresence, ciWorkflowPath);
        Assert.DoesNotContain("Run local NuGet smoke feed preparation", ciWorkflow, StringComparison.Ordinal);
        Assert.DoesNotContain("Clear local Agibuild NuGet cache", ciWorkflow, StringComparison.Ordinal);
        Assert.DoesNotContain("dotnet publish tests/Agibuild.Fulora.Integration.NugetPackageTests/Agibuild.Fulora.Integration.NugetPackageTests.csproj", ciWorkflow, StringComparison.Ordinal);

        Assert.DoesNotContain("PrepareNugetPackageSmokeFeed", buildCs, StringComparison.Ordinal);
        AssertSourceContains(packagingSource, "internal Target PrepareNugetPackageSmokeFeed", RuntimeCriticalPathScenarioPresence, packagingPath);
        AssertSourceContains(packagingSource, "internal Target ValidateLinuxNativeAotNugetPackagePublish", RuntimeCriticalPathScenarioPresence, packagingPath);
        AssertSourceContains(packagingSource, ".DependsOn(PrepareNugetPackageSmokeFeed)", RuntimeCriticalPathScenarioPresence, packagingPath);
        AssertSourceContains(packagingSource, "Clearing NuGet cache", RuntimeCriticalPathScenarioPresence, packagingPath);
        AssertSourceContains(packagingSource, ".SetRuntime(\"linux-x64\")", RuntimeCriticalPathScenarioPresence, packagingPath);
        AssertSourceContains(packagingSource, ".SetProperty(\"PublishAot\", \"true\")", RuntimeCriticalPathScenarioPresence, packagingPath);
        AssertSourceContains(packagingSource, ".DependsOn(Build)", RuntimeCriticalPathScenarioPresence, packagingPath);
        AssertSourceContains(packagingSource, "PackNugetSmokeFeedProjects", RuntimeCriticalPathScenarioPresence, packagingPath);
    }
    ...
    public void Readme_quality_signals_match_actual_test_evidence()
    {
        ...
        AssertSourceContains(readme, "capability tiers", ReadmeQualitySignals, readmePath);
    ...
    public void Ci_evidence_snapshot_build_target_emits_v2_schema_with_provenance()
    {
        ...
        AssertAnonymousObjectHasMembers(
            combinedSource,
            ["tests", "coverage", "governance", "distributionReadiness", "adoptionReadiness"],
            EvidenceContractV2Schema,
            "build/Build.Governance.Release.cs");
        AssertSourceContains(combinedSource, "ReleaseCloseoutProvenanceInvariantId", EvidenceContractV2Schema, "build/Build.Governance.Release.cs");
