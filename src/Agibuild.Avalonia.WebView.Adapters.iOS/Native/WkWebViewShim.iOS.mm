// Native WKWebView shim for iOS (Objective-C++)
// Exposes a stable C ABI for use from net10.0-ios via P/Invoke (__Internal).
//
// Build requirements:
// - clang++ with -fobjc-arc, iOS SDK
// - links against UIKit + WebKit + Foundation
...
bool ag_wk_attach(ag_wk_handle handle, void* uiview_ptr);
...
// Environment options — call before ag_wk_attach.
...
struct shim_state
{
    ...
    __strong UIView* parent_view { nil };
...
static void run_on_main(void (^block)(void))
{
    ...
    // UIKit/WebKit APIs must be called on the main thread.
...
- (void)webView:(WKWebView*)webView requestMediaCapturePermissionForOrigin:(WKSecurityOrigin*)origin initiatedByFrame:(WKFrameInfo*)frame type:(WKMediaCaptureType)type decisionHandler:(void (^)(WKPermissionDecision))decisionHandler API_AVAILABLE(ios(15.0))
{
    ...
    if (s->callbacks.on_permission)
    {
        ...
        switch (type)
        {
            case WKMediaCaptureTypeCamera: kind = 1; break;
            case WKMediaCaptureTypeMicrophone: kind = 2; break;
            case WKMediaCaptureTypeCameraAndMicrophone: kind = 1; break;
        }
        ...
        if (state == 1) { decisionHandler(WKPermissionDecisionGrant); return; }
        if (state == 2) { decisionHandler(WKPermissionDecisionDeny); return; }
    }
...
{
    shim_state* s = self.state;
    if (s == nullptr || s->detached.load() || s->callbacks.on_scheme_request == nullptr)
    {
...
bool ag_wk_attach(ag_wk_handle handle, void* uiview_ptr)
{
    if (!handle || !uiview_ptr) return false;
    auto* s = (shim_state*)handle;
    ...
    run_on_main(^{
        @autoreleasepool
        {
            ...
            UIView* parent = (__bridge UIView*)uiview_ptr;
            if (parent == nil)
            {
                ok = false;
                return;
            ...
            // Ephemeral data store (non-persistent cookies/storage).
            if (s->opt_ephemeral)
            {
            ...
            s->web_view.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;

            // DevTools (inspectable) — requires iOS 16.4+.
...
void ag_wk_capture_screenshot(ag_wk_handle handle, ag_wk_screenshot_cb callback, void* context)
{
    ...
    run_on_main(^{
        ...
        [s->web_view takeSnapshotWithConfiguration:config completionHandler:^(UIImage* _Nullable image, NSError* _Nullable error) {
...
double ag_wk_get_zoom(ag_wk_handle handle)
{
    ...
    // iOS WKWebView doesn't have pageZoom; use JavaScript document.body.style.zoom
    // For simplicity, store zoom in a tag-like mechanism via JS
    return 1.0; // Placeholder; real zoom tracked via JS
}
...
void ag_wk_set_zoom(ag_wk_handle handle, double zoom_factor)
{
    ...
    run_on_main(^{
        ...
        // iOS WKWebView lacks pageZoom; apply via CSS zoom
        NSString* script = [NSString stringWithFormat:@"document.body.style.zoom='%f'", zoom_factor];
        [s->web_view evaluateJavaScript:script completionHandler:nil];
    });
}
...
const char* ag_wk_add_user_script(ag_wk_handle handle, const char* js)
{
    ...
    NSString* scriptId = [NSString stringWithFormat:@"preload_%lld", id];

    [s->web_view.configuration.userContentController addUserScript:script];
    return strdup(scriptId.UTF8String);
}
